...
function priorityColor(task, todo) {
  if (task.priority == 0) {
    todo.style.borderLeftColor = "green";
  } else if (task.priority == 1) {
    todo.style.borderLeftColor = "orange";
  } else if (task.priority == 2) {
    todo.style.borderLeftColor = "red";
  }
}
...
function restore() {
  ...
  if (retrievedObject !== null) {
    for (let i = 0; i < retrievedObject.list.length; i++) {
      ...
      temp.todosList = JSON.parse(
        JSON.stringify(retrievedObject.list[i].todosList)
      );
